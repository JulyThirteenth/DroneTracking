...
P_FSM_NODE="$(tmux display-message -p -t "${SESSION}":0.0 "#{pane_id}")"
P_BOTTOM="$(tmux split-window -v -p 35 -t "${P_FSM_NODE}" -P -F "#{pane_id}")"
P_FSM_APP="$(tmux split-window -v -p 50 -t "${P_FSM_NODE}" -P -F "#{pane_id}")"
