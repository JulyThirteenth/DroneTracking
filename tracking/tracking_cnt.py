...
class TrackerCtbrBase(ABC):
    controller = "base"

    def __init__(self, cfg: TrackingConfig = DEFAULT_CONFIG, solver: str | None = None):
        self.cfg = cfg
        self.solver = str(solver or cfg.default_solver).lower().strip()
        self.cnt_dt = float(cfg.control.dt)
        self.mpc_dt = float(cfg.mpc.dt)
        ...
        self._yaw_rate_limit = (
            np.inf if cfg.yaw.yaw_rate_limit is None else float(cfg.yaw.yaw_rate_limit)
        )
    ...
    def step(
        ...
        **kwargs,
        ...
        if self._warm_started:
            self._a_est = self._a_est + self._u_last * self.cnt_dt
        else:
            self._a_est = a_enu
        ...
        x_sol, u_sol, ref_traj = self._solve(x0, u0, log_solver=log_solver, **kwargs)
        a_enu = x_sol[6:9, 1]
        jerk_enu = u_sol[:, 0]
        yaw_rate_enu, yaw_rate_ff_enu, yaw_rate_fb_enu, yaw_err_enu = (
            self._yaw_cmd_calc(
                yaw_enu=yaw_enu,
                yaw_cmd_enu=yaw_cmd_enu,
            )
        )
    ...
    def _yaw_cmd_calc(self, *, yaw_enu: float, yaw_cmd_enu: float):
        yaw_rate_ff_enu = (
            0.0
            if self._yaw_cmd_prev_enu is None
            else wrap_pi(yaw_cmd_enu - self._yaw_cmd_prev_enu) / self.cnt_dt
        )
        yaw_err_enu = wrap_pi(yaw_cmd_enu - yaw_enu)
        yaw_rate_fb_enu = self._yaw_kp * yaw_err_enu / self.cnt_dt
        ...
        return yaw_rate_enu, yaw_rate_ff_enu, yaw_rate_fb_enu, yaw_err_enu
    ...
    @abstractmethod
    def _solve(self, x0, u0, *, log_solver: bool, **kwargs):
        pass


class MPCTrackerCtbr(TrackerCtbrBase):
    controller = "mpc"

    def __init__(self, cfg: TrackingConfig = DEFAULT_CONFIG, solver: str | None = None):
        super().__init__(cfg=cfg, solver=solver)
        params = make_mpc_params(cfg, with_dynamics=self.solver not in {"osqp"})
    ...
    def _solve(
        ...
        *,
        ...
        p = x0[:3]
        ref_traj = (
            np.repeat(p.reshape(3, 1), self.horizon + 1, axis=1)
            if ref_traj_enu is None
            else np.asarray(ref_traj_enu, dtype=float)
        )
        ...
        self._x_ws = x_sol
        self._u_ws = u_sol
        return x_sol, u_sol, ref_traj


class MPCCTrackerCtbr(TrackerCtbrBase):
    controller = "mpcc"

    def __init__(self, cfg: TrackingConfig = DEFAULT_CONFIG, solver: str | None = None):
        super().__init__(cfg=cfg, solver=solver)
        params = make_mpcc_params(cfg, with_dynamics=self.solver not in {"osqp"})
        if self.solver in {"osqp"}:
            from tracking.tracking_osqp import MPCCOSQP

            self._opt = MPCCOSQP(params)
        else:
            from tracking.tracking_opt import MPCC

            self._opt = MPCC(params)
        self._opt.setup()
        self._x_ws = np.zeros((params.nx, params.horizon + 1))
        self._u_ws = np.zeros((params.nu, params.horizon))
    ...
    def reset_warmstart(self):
        super().reset_warmstart()
        self._s_ws.fill(0.0)
        self._vs_ws.fill(0.0)

    def _solve(self, x0, u0, *, log_solver: bool, path_points_enu=None):
        if path_points_enu is None:
            raise ValueError("MPCC step requires path_points_enu.")
        path_points = np.asarray(path_points_enu, dtype=float).reshape(-1, 3)
        target = path_points[0] if path_points.shape[0] < 2 else path_points
